...
For `CloseClaw`, custom skills should live under the active workspace `skills/` directory so they can be discovered automatically at runtime (for example, `<workspace_root>/skills/my-skill/SKILL.md`).
...
Keep frontmatter minimal. In `CloseClaw`, `metadata` and `always` are also supported when needed, but avoid adding extra fields unless they are actually required.
...
---
> Source: [2001Haru/CloseClaw](https://github.com/2001Haru/CloseClaw) — distributed by [TomeVault](https://tomevault.io).
