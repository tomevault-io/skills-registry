---
name: milkio
description: milkio，后端，状态管理，Electron主进程，数据仓库，缓存，逻辑，测试；*.action.ts *.stream.ts __TEST__.test.ts *.config.ts *.table.ts *.cache.ts *.code.ts *.codes.ts *.event.ts *.handler.ts *.seed.ts Use when this capability is needed.
metadata:
  author: akirarika
---
...
---
> Source: [akirarika/milkio](https://github.com/akirarika/milkio) — distributed by [TomeVault](https://tomevault.io).
