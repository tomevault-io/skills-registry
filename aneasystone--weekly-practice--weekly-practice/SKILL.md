---
name: writing-style
description: 模仿作者的技术博客写作风格，生成中文技术系列文章。当用户要求写博客、写文章、写技术教程时使用此技能。 Use when this capability is needed.
metadata:
  author: aneasystone
---
...
每个系列的文章统一存放在一个**以项目命名的系列目录**下，目录名采用项目 GitHub 仓库的 `owner__repo` 形式（owner 和 repo 之间用双下划线 `__` 连接）。系列目录下，每篇文章是一个以**数字序号**开头的子目录：
...
1. **系列目录以项目的 `owner__repo` 命名**，例如 `obra__superpowers`、`NousResearch__hermes-agent`、`sansan0__TrendRadar`。如果 owner 或 repo 名本身含斜杠或其它特殊字符，统一用 `__` 替换
2. **每篇文章是一个子目录**，而不是一个文件。目录命名格式为 `NN-{主题}-{子主题}`，其中 `NN` 是两位数字序号（`01`、`02`……不足两位补零），按系列阅读顺序从 `01` 依次递增
3. 文章正文写在子目录下的 `README.md` 中
4. 子目录下创建 `images/` 目录用于存放截图
5. **不再使用日期作为目录名**——序号只表示文章在系列中的先后顺序，与发布日期无关。这样草稿阶段不必关心日期，正式发布时也无需为了对齐发布日期而改名

例如，写一个关于 RAGFlow 的系列，首篇快速入门应创建：
- `infiniflow__ragflow/01-ragflow-quickstart/README.md`（文章正文）
- `infiniflow__ragflow/01-ragflow-quickstart/images/`（截图目录）

### 2.3 篇幅与序号

每篇文章的篇幅适中，通常 200~500 行 Markdown。如果某个主题内容较多，会拆分为多篇，序号顺延即可（如 `07-xxx-source-1`、`08-xxx-source-2`），不需要再在标题或目录里另加 `-2`、`-3` 后缀来区分。

**序号规则：**

1. 系列第 1 篇用 `01`，第 N 篇用零补齐的两位数字 `NN`，按系列阅读顺序依次递增
2. **序号必须连续、唯一，不得跳号或复用**：一个系列内不能出现两个 `03`，也不能从 `03` 跳到 `05`
3. 序号只反映文章在系列里的先后顺序，**与发布日期无关**——批量生成整个系列初稿时，直接按规划顺序从 `01` 编到 `NN` 即可，不用预占日期，也不用担心是串行还是并行产出。每篇天然落在各自的序号目录下，不会出现"挤在同一天"的问题

> 正文里"昨天我们学习了……""我们明天继续"这类衔接语仍可照常使用（见 3.2、3.4 节），它表达的是阅读/发布节奏，与目录是否带日期无关。
...
- `不过百闻不如一见，百见不如一试，……我们明天就来看看 Mem0 的代码。`
...
如果给读者一句**英文提示词**让他照着用（比如丢给 AI 编程助手的指令），在代码块里紧跟一行 `#` 注释给出中文翻译，方便读者理解意图：
...
两类词在第一次出现时都要**就地解释**，否则读者会卡壳：

1. **源码里遇到的不熟悉技术**（如 Redis Stream、Leiden 算法、Trio）：用一整段甚至一小节从零讲清楚，必要时配示例代码验证，再回到主叙事线。
2. **任何英文术语 / 行业黑话**（如 props、explainer、motion graphics、字幕烧录、ffprobe、CLIP、GSAP、Ken Burns 效果，以及文章涉及的各种第三方库 / 工具名）：第一次出现就用一句话或一个引用块解释它是什么、干嘛用的，尽量给中文译名（见六节），能加官方链接的加上并登记到 `## 参考`（3.5 节）。
...
**解释必须技术正确**：给术语打的比方要经得起推敲，确认这个例子体现的是该技术**独有**的能力，而不是某个更基础的技术也能做的事。举个容易犯的错：解释跨模态检索模型（如 CLIP）时，举例「让『一段海浪的镜头』和『ocean waves at dusk』落在相近位置」——但这两个**都是文字**，是普通文本 embedding 就能做的，没体现「图片↔文字映射到同一空间」这个独有能力。正确的例子应该一端是图、一端是文字。
...
下面几类是生成文章时最容易冒出来的，单独列出来：

- **给事实贴主观程度词**（让数字/事实自己说话）：`老老实实标了成本`、`数字低得出乎意料`、`这个区别很要命`、`是里面最重的依赖`、`装起来不轻`、`相当吃力`。判定：把形容词/副词删掉句子照样成立，就删掉。比如「每条都老老实实标了成本，数字低得出乎意料」→「每条都标了成本」，后面的数字自己会说话
- **给引文预先下判断的引出语**：`有一句话说得很直白`、`专门点了一句`、`一针见血地指出`、`说得很到位`。引文够不够直白让读者自己看，引出时一律用中性说法：`……里是这样描述的：` / `原文如下：` / `里有这么一句：`
- **表态式的喜好**：`X 有一个我很喜欢的设计：……`、`我个人很欣赏的一点是……`。直接说这个设计是什么，不要先表一句态
- **填充式提示语**（空转、没信息量）：`这里有一点要注意`、`值得留意的是`、`这里其实有个细节`。删掉直接讲事实，或只用 `不过` / `其实` 这种一个词的转折
- **下结论式金句**：`一个诚实的结论是……`、`真正最轻的那条路恰恰是……`。把价值判断删掉，事实留下
...
- 偶尔使用口语化表达增加亲切感：`瞅了一眼`、`瞬间就不香了`、`这不得好好研究下它的代码嘛`、`再也不用担心 API 费用把钱包掏空了`
- **口语化的动词比喻，能用平实准确的词就用平实的**：作者偏好平实准确，不喜欢生造的口语动词。这类都该换掉：`慢慢扒`（→ 慢慢展开/细看）、`组件吃下数据`（→ 接收）、`把 X 死死钉在 Y`（→ 锁定为）、`趁热瞄一眼源码`（→ 顺便看一眼）、`请求打到接口`（→ 调用接口）、表格表头 `动起来靠`（→ 运动来源）。判定：一个口语动词如果牺牲了准确性、或读着像硬凑的「网感」，就换成平实的说法。`扒一扒`「钉死」「吃下」这类一律避免
...
- **概念先铺垫、再引用，不要凭空甩**：不要把一个读者还没见过的概念/流程/字段，当成已知的东西直接拿来用。两种典型踩法：① 突然甩出一串阶段名（如 `idea → scene_plan → … → compose`），但前文从没讲过它分哪几个阶段；② 引用一个只在某段 JSON / 配置里一闪而过的字段（如某个 `xxx_runtime`），却没说它是什么、谁来定。**正确做法**：第一次用到某个流程/字段时，先用半句话交代它是什么（`这个字段指定一支视频最终用哪个引擎渲染`），或明说「细节后面单开一篇讲，这里只需知道……」，再往下展开。判定：每引用一个 `代码字段` / 阶段名 / 专有流程，回看前文——读者在这一行之前是否已经知道它是什么？不知道就先补一句。
- **概念的归属范围要准**：别把一个平台级 / 通用的东西说成某个场景专属。比如把整个平台都有的某个组件，说成「免费版专用的 XX」「这条路径专用的 XX」，但它其实和这个限定无关。下笔前问一句：这个限定词（「免费的」「这条路径的」）是不是真的成立，还是顺手加的。
...
- **正文里不要出现「README」这个词**：要引用项目主页 / 仓库首页的内容，说「项目介绍」「官方介绍」「项目首页」或「上一篇提到的……」。`README.md` 只是博客文件的固定文件名，不是给读者看的内容。`## 参考` 里也别单列一条「XXX README」，用「XXX GitHub 仓库」即可
- **机器相关的数字不要当通用结论写**：渲染耗时、下载用时这类跟硬件强相关的数字，写进正文当结论会误导（A 机器 80 秒不代表 B 机器也 80 秒）。真实终端输出（代码块）里原样保留没问题；但用文字复述时只做定性描述（「首次要完整构建，后续走缓存」），不要写「大约 80 秒」。文件大小、压缩比这种与机器无关的数字可以留
...
5. **编辑腔句式排查**：grep 下列词，找到就重写或删除（完整清单见 5.0.5）
   - `更有意思的是` / `值得一提的是` / `值得留意的是` / `这里有一点要注意` / `另外顺带一提` / `把这件事说得直白一点` / `换句话说` / `一句话定义`
   - 主观程度词：`老老实实` / `出乎意料` / `很要命` / `最重的` / `装起来不轻` / `有一个我很喜欢的设计` / `一个诚实的结论是`
   - 给引文贴评价的引出语：`说得很直白` / `专门点了一句` / `一针见血`
   - `最后拼的就是` / `堆是堆不出来的` / `已经从 X 变成了 Y` 式金句
6. **价值判断与事实的比例**：每一段里，作者的主观评价不能超过事实句的数量。宁可少评价、多数据
7. **整句加粗的 slogan**：搜 `**...。**`，把整句话加粗的句子改回普通陈述
8. **口语动词比喻排查**：grep `扒` / `钉死` / `吃下` / `瞄一眼` / `打到`，凡是牺牲准确性或硬凑网感的，换成平实说法（见 5.1）
9. **开头"本文目标"套路句排查**（系列首篇必查）：有没有"先讲清楚 X 是什么、再动手跑起来""本文将先介绍……再带大家上手"这种预告章节结构的句子。把项目名一换还成立，就是套路句，删掉或换成扣住本项目卖点的好奇钩子（见 3.2）

### 10.3 技术正确性

10. **介绍篇开头检查**：如果用了模式 B（热点切入），热点是否真实可搜到、有具体来源；如果用的是模式 A/C 或没用热点，开头里是否包含足够多具体可验证的事实（产品名、版本号、日期、API 名等）；任何模式都要避开"教科书式"的空洞趋势话术（4.0 节 / 3.2 节）
11. 介绍篇是否避开了贬损性热点（3.2 节）
12. **关键事实当场核验，不靠搜索摘要或记忆**：
    - **日期**：项目「开源日期」用 `git log` 第一个 commit 或 GitHub API 的 `created_at`，别把媒体报道日 / 文章发布日当成开源日
    - **star / fork 数**：用 GitHub API（`stargazers_count`），别用搜索摘要里的数字（搜索摘要常不准，甚至虚高一倍）
    - **作者是否值得点名**：账号新、followers 少、没别的知名作品的个人开发者，默认不写「作者是某某」（见 3.2）
    - **给术语打的比方是否正确**：确认这个例子体现的是该技术**独有**的能力，而不是更基础的技术也能做的事（见 4.8 的反例）
13. **黑话 gloss 检查**：通读全文，每个英文术语 / 缩写第一次出现是否都解释了（`props`、`explainer`、`motion graphics`、`烧录`、`ffprobe` 这类最容易漏，见 4.8）
14. **概念铺垫检查**：每个 `代码字段` / 阶段名 / 专有流程，读者在该行之前是否已经知道它是什么；限定词（「免费的」「这条路径的」）的归属范围是否准确（见 5.4）
15. **「README」「机器相关数字」检查**：正文不出现「README」；渲染耗时等机器相关数字不当通用结论写（见六节）
16. **英文提示词是否配了中文翻译行**（见 4.1）
17. `## 参考` 是否登记了所有正文外链（3.5 节）
18. 目录是否用 `owner__repo/NN-slug` 格式、序号从 `01` 起连续递增且无跳号或复用（2.2 / 2.3 节）

**改稿一致性**：删一节、移一节后，回头检查 `## 小结` 的对应要点、以及别处对它的交叉引用（「上面那节」「后面会讲」）是否还成立，别留悬空指代。

**一句话判据**：读者读完这段，脑子里留下的应该是**新的事实 / 新的代码 / 新的数据**，而不是**作者又评价了一番**。如果删掉某句话段落照样成立，基本就该删。

---
> Source: [aneasystone/weekly-practice](https://github.com/aneasystone/weekly-practice) — distributed by [TomeVault](https://tomevault.io).
