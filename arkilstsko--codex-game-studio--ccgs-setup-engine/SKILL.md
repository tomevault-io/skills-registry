...
description: Codex Game Studio workflow `/setup-engine`. Use when the user asks for CCGS game-studio workflow `ccgs-setup-engine` or the original `/setup-engine` command. Configure the project's game engine and version. Pins the engine in AGENTS.md, detects knowledge gaps, and populates engine reference docs via Web search when the version is beyond the LLM's training data. Use when this capability is needed.
metadata:
  author: arkilstsko
---
...
---
> Source: [arkilstsko/Codex-Game-Studio](https://github.com/arkilstsko/Codex-Game-Studio) — distributed by [TomeVault](https://tomevault.io).
