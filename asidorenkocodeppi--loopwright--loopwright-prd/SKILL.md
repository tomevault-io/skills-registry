---
name: loopwright-prd
description: Generate a Product Requirements Document (PRD) for loopwright task orchestration. Creates PRDs with user stories that can be converted to beads issues or prd.json for automated execution. Triggers on: create a prd, write prd for, plan this feature, requirements for, spec out. Use when this capability is needed.
metadata:
  author: asidorenkocodeppi
---

# Loopwright PRD Generator

Create detailed Product Requirements Documents optimized for AI agent execution via loopwright.
...
This section is extracted by conversion tools (loopwright-create-json, loopwright-create-beads) and appended to each story's acceptance criteria.
...
The PRD will be executed by AI coding agents via loopwright. Therefore:
...
[PRD]
# PRD: Dark Mode for Loopwright

## Overview
Add dark mode support to loopwright to reduce eye strain during long orchestration sessions. The feature will apply to all TUI components and persist the user's preference in the config file.
...
**Acceptance Criteria:**
- [ ] Add `theme` field to `.loopwright.yaml` schema
...
- FR-1: Theme setting must be readable from `.loopwright.yaml`
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/asidorenkocodeppi) — claim your Tome and manage your conversions.
