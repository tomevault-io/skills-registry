---
name: jira
description: Natural language interaction with Jira for managing issues, sprints, and workflows using jira CLI or Atlassian MCP. Use when the user mentions Jira tickets, issue keys, sprints, backlogs, or asks to create or modify Jira issues. Use when this capability is needed.
metadata:
  author: bakwankawa
---
...
This README is used as a Codex project skill under `lazy-code-skill/jira` in this repository.
...
In this repository, `references/commands.md` and `references/mcp.md` are minimal placeholders that point to the upstream Jira skill references in `softaworks/agent-toolkit`. Consult those upstream files for the canonical, detailed reference.
