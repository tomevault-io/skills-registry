...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-kb-indexing` | KB 비동기 인덱싱 파이프라인 검증 | `backend/**/knowledge/*.java`, `backend/**/AsyncConfig.java`, `backend/**/DocumentTextExtractor.java`, `backend/**/DocumentMetadataAnalyzer.java` |
| `verify-flyway-migrations` | Flyway DB 마이그레이션 일관성 검증 | `backend/**/db/migration/V*.sql`, `backend/**/persistence/**/*JpaEntity.java`, `backend/**/persistence/metrics/*.java` |
...
| `verify-rag-pipeline` | RAG 파이프라인 검증 | `backend/**/answer/**/*.java`, `backend/**/analysis/*.java`, `backend/**/search/*.java`, `backend/**/vector/*.java`, `backend/**/rag/**/*.java`, `backend/**/document/DocumentDownloadController.java`, `backend/**/prompt/*.java`, `backend/**/resources/prompts/*.txt` |
