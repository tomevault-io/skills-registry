---
name: gui-change
description: Mandatory before ANY host console change under host-console/. Documents the dead-code map (domain/, platform/, styles.css, App.css), canonical types.ts surface, transport classes (Mock/WebSerial/WebSocket), and common mistakes. Trigger on edits to host-console/src/**, component additions, transport changes, React hooks edits, CSS changes, or workspace layout changes. Load alongside Uncodixfy for design-language rules. Use when this capability is needed.
metadata:
