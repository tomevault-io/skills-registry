...
```bash
bash .codex/skills/dotnet-test-triage/scripts/run-test-triage.sh ./infra-monitor.slnx --no-restore
```

Optional: override the default command (useful for repo-specific defaults):
```bash
DOTNET_TEST_CMD="dotnet test ./infra-monitor.slnx --no-restore" \
  bash .codex/skills/dotnet-test-triage/scripts/run-test-triage.sh
```
