...
Use this skill only inside the `gemini:gemini-delegate` subagent.
...
- Do not call `setup`, `review`, `search`, `analyze`, `status`, `result`, or `cancel` from `gemini:gemini-delegate`.
...
Parameter mappings:
- `--model <value>`: passed through to `gemini-companion.mjs task --model <value>`. Preserves the user's model choice without modification.
- `--resume-last`: appended to the `task` command when the user requests continuation of previous work.
- `--background`: controls whether the Bash call runs in the background. Not passed to `task`.
- `--wait`: controls whether to wait for background completion. Not passed to `task`.
...
---
> Source: [bsmi021/ai-plugins-cc](https://github.com/bsmi021/ai-plugins-cc) — distributed by [TomeVault](https://tomevault.io).
