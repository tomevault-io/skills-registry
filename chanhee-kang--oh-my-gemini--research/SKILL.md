---
name: research
description: Orchestrate parallel scientist agents for comprehensive research with AUTO mode Use when this capability is needed.
metadata:
  author: chanhee-kang
---
...
```
/oh-my-gemini:research <goal>                    # Standard research with user checkpoints
/oh-my-gemini:research AUTO: <goal>              # Fully autonomous until complete
/oh-my-gemini:research status                    # Check current research session status
/oh-my-gemini:research resume                    # Resume interrupted research session
/oh-my-gemini:research list                      # List all research sessions
/oh-my-gemini:research report <session-id>       # Generate report for session
```

### Quick Examples

```
/oh-my-gemini:research What are the performance characteristics of different sorting algorithms?
/oh-my-gemini:research AUTO: Analyze authentication patterns in this codebase
/oh-my-gemini:research How does the error handling work across the API layer?
```
...
Task(subagent_type="oh-my-gemini:scientist-low", model="gemini-flash-latest", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Task(subagent_type="oh-my-gemini:scientist", model="gemini-pro-latest", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Task(subagent_type="oh-my-gemini:scientist-high", model="gemini-pro-latest", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
Task(subagent_type="oh-my-gemini:scientist", model="gemini-pro-latest", prompt="
...
```
/oh-my-gemini:research AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="oh-my-gemini:scientist-low", model="gemini-flash-latest", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="oh-my-gemini:scientist-low", model="gemini-flash-latest", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="oh-my-gemini:scientist-low", model="gemini-flash-latest", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="oh-my-gemini:scientist", model="gemini-pro-latest", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="oh-my-gemini:scientist", model="gemini-pro-latest", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="oh-my-gemini:scientist", model="gemini-pro-latest", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="oh-my-gemini:scientist-high", model="gemini-pro-latest", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/oh-my-gemini:research status` | Show current session progress |
| `/oh-my-gemini:research resume` | Resume most recent interrupted session |
| `/oh-my-gemini:research resume <session-id>` | Resume specific session |
| `/oh-my-gemini:research list` | List all sessions with status |
| `/oh-my-gemini:research report <session-id>` | Generate/regenerate report |
| `/oh-my-gemini:research cancel` | Cancel current session (preserves state) |
...
Optional settings in `.gemini-cli/settings.json`:
