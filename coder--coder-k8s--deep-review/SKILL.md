...
- Identify the change scope using an explicit revision range (or the file list the user provides), for example `BASE="$(git merge-base origin/main HEAD)"` then `git diff --name-only "${BASE}...HEAD"`.
- Skim the diff for intent and risk over the same range, for example `git diff "${BASE}...HEAD"`.
