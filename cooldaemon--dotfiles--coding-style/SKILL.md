...
- [ ] Data + functions preferred over stateful classes
...
ALWAYS use each language's strongest immutability mechanism. Create new values, NEVER mutate existing ones:
...
## Safety-First Type Design

### Wrap Primitives in Domain Types

Create domain-specific types that validate on construction. Never pass raw primitives (string, int) for domain concepts like email, age, or currency. Use a module-level constructor function or factory that returns an immutable domain type.

### Encode Valid States as Types

Use sum types or sealed hierarchies instead of boolean flags or string status fields. Make invalid states unrepresentable.

### Eliminate Null at Type Boundaries

Use the language's null-safety mechanism. Never allow null to propagate silently through domain logic — make optionality explicit in type signatures.

### Language Mechanisms

| Principle | TypeScript | Python | Go | Ruby | Rust | C | C++ |
|-----------|-----------|--------|-----|------|------|---|-----|
| Domain types | Class + private constructor + factory | `@dataclass(frozen=True)` + factory function | Named type + constructor | `Data.define` + `initialize` | Newtype (`struct Email(String)`) | `typedef` + constructor function | Value class + factory |
| Sum types | Discriminated union | Dataclass variants + `match` | Interface + type switch | `Data.define` + pattern matching | `enum` | Tagged union | `std::variant` |
| Null safety | `T \| undefined` | `T \| None` | `(*T, error)` with sentinel error | `T?` (nilable) + guard | `Option<T>` | Explicit NULL check | `std::optional<T>` |

## Data + Functions over Stateful Classes
...
For external boundaries (API, user input), use schema validation (Zod, Pydantic). For internal boundaries (domain logic), use domain types that validate on construction (see "Wrap Primitives in Domain Types" above).
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/cooldaemon) — claim your Tome and manage your conversions.
