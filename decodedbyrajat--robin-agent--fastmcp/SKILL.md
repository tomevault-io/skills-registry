...
```bash
python ~/.robin/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.robin/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
- configure the server in `~/.robin/config.yaml` using the `native-mcp` skill, or
...
The server-building part may be correct while the Robin config is not. Load the `native-mcp` skill and configure the server in `~/.robin/config.yaml`, then restart Robin.
...
---
> Source: [decodedbyrajat/Robin-agent](https://github.com/decodedbyrajat/Robin-agent) — distributed by [TomeVault](https://tomevault.io).
