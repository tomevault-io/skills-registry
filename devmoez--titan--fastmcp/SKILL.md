...
- smoke-test a server with the FastMCP CLI before wiring it into Titan or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Titan. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.Titan/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.Titan/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Titan integration, either:

- configure the server in `~/.Titan/config.yaml` using the `native-mcp` skill, or
...
### Titan cannot see the deployed server

The server-building part may be correct while the Titan config is not. Load the `native-mcp` skill and configure the server in `~/.Titan/config.yaml`, then restart Titan.
...
---
> Source: [devMoez/titan](https://github.com/devMoez/titan) — distributed by [TomeVault](https://tomevault.io).
