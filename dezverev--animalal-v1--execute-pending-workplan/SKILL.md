...
1. List `.cursor/workplans/Pending/*.md` (create `.cursor/workplans/Pending/` if missing). If the list is empty, report "No workplans in Pending" and stop.
...
1. Ensure `.cursor/workplans/Inprogress/` exists.
2. Move the chosen file from `.cursor/workplans/Pending/<slug>.md` to `.cursor/workplans/Inprogress/<slug>.md` (no content change).
...
2. For each numbered step:
   ...
   - Read `.cursor/skills/<skill-name>/SKILL.md` and follow that skill's workflow (discovery, edits, commands, etc.).
...
1. **Run smart-test-runner**: Scope from the plan's "Scope / Affected areas" or "Key files" → map to tags (or "run all" if kernel/wide). Read `.cursor/skills/smart-test-runner/SKILL.md` and follow its workflow. Execute from repo root: `.\run-tests.ps1` with `-t <tags>` when filtered, or no `-t` for full suite.
2. **Run check-test-logical-resp** on the latest test-results JSON in `src/IntegrationTesterApp/test-results/`. Read `.cursor/skills/check-test-logical-resp/SKILL.md` and follow its workflow.
...
1. Ensure `.cursor/workplans/Done/` exists. Move `.cursor/workplans/Inprogress/<slug>.md` → `.cursor/workplans/Done/<slug>.md`.
...
2. **Create continuation GitHub issue**:
   ...
   - Create a new GitHub issue with:
     ...
     - **Body**: Markdown that includes:
       ...
       - Link to progress report (`.cursor/workplans/DoneReport/<slug>_PROGRESS.md`)
       - Link to in-progress workplan (`.cursor/workplans/Inprogress/<slug>.md`)
...
1. Ensure `.cursor/workplans/DoneReport/` exists. Create it if it does not.
2. Create report based on outcome:
   - **Full Success**: Create `.cursor/workplans/DoneReport/<slug>_Report.md` - Summary of what was done, references used, decisions made.
   - **Partial Success**: Create `.cursor/workplans/DoneReport/<slug>_PROGRESS.md` - Detailed progress report including:
   ...
   - **Failure**: Create `.cursor/workplans/DoneReport/<slug>_FAILURE.md` - Summary of what was attempted, what failed, error details, recommended fixes.
...
- **Resume**: If the user says "continue the workplan" and there is exactly one file in `.cursor/workplans/Inprogress/`, skip Step 1–2 and proceed from Step 3 using that plan.
