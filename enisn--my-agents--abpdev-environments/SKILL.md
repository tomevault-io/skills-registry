...
SqlServer:
  variables:
    ConnectionStrings__Default: "<set a local connection string with credentials outside git>"
...
sqlserver:
  start-cmds:
    ...
    - docker run --name tmp-sqlserver --restart unless-stopped -e "ACCEPT_EULA=Y" -p 1433:1433 -d mcr.microsoft.com/mssql/server:2017-CU8-ubuntu
  stop-cmds:
...
---
> Source: [enisn/my-agents](https://github.com/enisn/my-agents) — distributed by [TomeVault](https://tomevault.io).
