...
description: Reusable recipe for calling any BFL FLUX endpoint that follows the async POST → poll pattern. Use when integrating a new endpoint not covered by the existing BaseFlux methods, or when explaining how the existing flow works. Use when this capability is needed.
metadata:
  author: gelasdev
---
