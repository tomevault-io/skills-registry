...
Defer route choice and lane selection to `../../reference/routing-guidance.md`. For `web/mobile UI`, keep `mobile-app` and `frontend-web` as the primary UI routes, start the primary skill lane with `Grok visual/product thinking`, use `frontend-ui-ux` only when stronger upstream product or interaction judgment helps, and add `impeccable` only as a supplementary refinement layer.
...
7. Run `review skill` after significant mobile changes.
...
- Use research tools or the built-in Explore behavior to match local navigation, screen, and component conventions.
- Use research tools, Context7, or equivalent knowledge retrieval for framework and platform APIs.
