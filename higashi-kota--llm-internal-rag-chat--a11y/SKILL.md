...
### 3.5. Missing id/name on form fields (Browser Autofill)

Form fields without `id` or `name` attributes prevent browser autofill and are flagged by Lighthouse:

```tsx
// BAD: Lighthouse warning "A form field element should have an id or name attribute"
<input type="text" placeholder="Search..." aria-label="Search" />

// GOOD: Include both id and name for autofill support
<input
  id="search-input"
  name="search-input"
  type="text"
...
**Testing for id/name attributes:**

```typescript
// accessibility.test.tsx
it("すべての input 要素に id または name 属性がある", () => {
  const { container } = render(<Component />)
  const inputs = container.querySelectorAll("input")

  for (const input of inputs) {
    const hasId = input.hasAttribute("id") && input.getAttribute("id") !== ""
    const hasName = input.hasAttribute("name") && input.getAttribute("name") !== ""
