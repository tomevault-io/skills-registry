...
**Language rule (vibe-kit override):** All user-facing text you generate (UI labels, error messages, button copy in the resulting app) defaults to Vietnamese for VN-built vibe-kit projects. Read `.vibe/intent.json` `raw` field to detect: VN prose → VN UI, English prose → English UI. Code, comments, file paths, npm commands stay English.
