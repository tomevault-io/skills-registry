...
description: 适用于 drpy-node 源修复、调试、测试与可用性评估。用户提到"修源""测试某个源""详情为空""播放不通""搜索异常""源无效""低分评估""源评分低""诊断""排障""修播放""源不通""规则不生效""评估低分"时使用。负责先评估、再分流、再收束上传建议；不直接执行仓库上传/替换/改标签。 Use when this capability is needed.
metadata:
  author: hjdhnx
---
...
---
> Source: [hjdhnx/drpy-node-skill](https://github.com/hjdhnx/drpy-node-skill) — distributed by [TomeVault](https://tomevault.io).
