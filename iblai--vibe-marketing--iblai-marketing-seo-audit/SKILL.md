...
Identify SEO issues and ship actionable recommendations to improve
...
Pin down before auditing:

1. **Site context**
   - Type (SaaS, e-commerce, blog, etc.)
   - Primary business goal for SEO
...
Reporting "no schema found" based solely on `web_fetch` or `curl` produces false audit findings — these tools can't see JS-injected schema.
...
**Check for:**
- Self-referencing entry on every page (the page must include itself in the hreflang set)
...
- `x-default` present, pointing to a fallback page (language selector or default locale)
...
- Best approach: don't create locale pages you can't make genuinely helpful
...
- Thin locale pages dragging down the site-wide quality signal
...
- [International SEO](references/international-seo.md): Evidence and sources for hreflang, canonical + i18n, sitemaps, URL structure, content quality across locales
- For AI search optimization (AEO, GEO, LLMO, AI Overviews), see `/iblai-marketing-ai-seo`
...
- **iblai-marketing-ai-seo**: Optimizing content for AI search engines (AEO, GEO, LLMO)
- **iblai-marketing-programmatic-seo**: Building SEO pages at scale
- **iblai-marketing-site-architecture**: Page hierarchy, navigation design, URL structure
- **iblai-marketing-schema-markup**: Implementing structured data
- **iblai-marketing-page-cro**: Optimizing pages for conversion (not just ranking)
- **iblai-marketing-analytics-tracking**: Measuring SEO performance
