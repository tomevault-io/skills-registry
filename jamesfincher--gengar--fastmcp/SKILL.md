...
- smoke-test a server with the FastMCP CLI before wiring it into Gengar or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Gengar. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.gengar/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.gengar/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Gengar integration, either:

- configure the server in `~/.gengar/config.yaml` using the `native-mcp` skill, or
...
### Gengar cannot see the deployed server

The server-building part may be correct while the Gengar config is not. Load the `native-mcp` skill and configure the server in `~/.gengar/config.yaml`, then restart Gengar.
...
---
> Source: [JamesFincher/gengar](https://github.com/JamesFincher/gengar) — distributed by [TomeVault](https://tomevault.io).
