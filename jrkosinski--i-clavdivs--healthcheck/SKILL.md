---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for i-clavdivs deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, i-clavdivs cron scheduling for periodic checks, or version status checks on a machine running i-clavdivs (laptop, workstation, Pi, VPS). Use when this capability is needed.
metadata:
  author: jrkosinski
---

# i-clavdivs Host Hardening

## Overview

Assess and harden the host running i-clavdivs, then align it to a user-defined risk tolerance without breaking access. Use i-clavdivs security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim i-clavdivs changes the host firewall, SSH, or OS updates; it does not.
...
5. i-clavdivs gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if i-clavdivs can access sensitive data.
...
### 2) Run i-clavdivs security audits (read-only)

As part of the default read-only checks, run `i-clavdivs security audit --deep`. Only offer alternatives if the user requests them:

1. `i-clavdivs security audit` (faster, non-probing)
2. `i-clavdivs security audit --json` (structured output)

Offer to apply i-clavdivs safe defaults (numbered):

1. `i-clavdivs security audit --fix`

Be explicit that `--fix` only tightens i-clavdivs defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check i-clavdivs version/update status (read-only)

As part of the default read-only checks, run `i-clavdivs update status`.
...
- Credential hygiene notes (location of i-clavdivs creds, prefer disk encryption)
...
- i-clavdivs security audit (re-run)
...
After i-clavdivs install or first hardening pass, run at least one baseline audit and version check:

- `i-clavdivs security audit`
- `i-clavdivs security audit --deep`
- `i-clavdivs update status`

Ongoing monitoring is recommended. Use the i-clavdivs cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `i-clavdivs cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `i-clavdivs update status`
...
Before creating, `i-clavdivs cron list` and match on exact `name`. If found, `i-clavdivs cron edit <id> ...`.
If not found, `i-clavdivs cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `i-clavdivs update status` (preferred for source checkouts and channels)
2. `npm view i-clavdivs version` (published npm version)

## i-clavdivs command accuracy

Use only supported commands and flags:

- `i-clavdivs security audit [--deep] [--fix] [--json]`
- `i-clavdivs status` / `i-clavdivs status --deep`
- `i-clavdivs health --json`
- `i-clavdivs update status`
- `i-clavdivs cron add|list|runs|run`

Do not invent CLI flags or imply i-clavdivs enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by i-clavdivs compaction:
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/jrkosinski) — claim your Tome and manage your conversions.
