...
```bash
SOCKET_DIR="${i-clavdivs_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/i-clavdivs-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/i-clavdivs.sock"
SESSION=i-clavdivs-python
...
- Use `i-clavdivs_TMUX_SOCKET_DIR` (legacy `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$i-clavdivs_TMUX_SOCKET_DIR/i-clavdivs.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `i-clavdivs_TMUX_SOCKET_DIR`).
