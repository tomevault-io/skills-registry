...
- Skill files are installed in `.claude/skills/ecc-continuous-learning-v2/`.
- Hook wiring in `.claude/settings.json` is intentionally not enabled in this task.
- To activate observation, add the hook config in the "Quick Start" section.
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "${CLAUDE_PLUGIN_ROOT}/skills/ecc-continuous-learning-v2/hooks/observe.sh pre"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
      "hooks": [{
        "type": "command",
        "command": "${CLAUDE_PLUGIN_ROOT}/skills/ecc-continuous-learning-v2/hooks/observe.sh post"
      }]
    }]
  }
}
