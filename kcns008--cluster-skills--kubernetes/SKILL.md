...
| **ARO** | 4.17.x | https://learn.microsoft.com/azure/openshift/ |
| **ROSA** | 4.17.x | https://docs.redhat.com/en/documentation/red_hat_openshift_service_on_aws/ |
...
| **rosa** | latest | ROSA cluster management |
| **aro** | latest (via az cli) | ARO cluster management |
...
**ARO:**
```bash
az aro show -g ${RG} -n ${CLUSTER} --query 'clusterProfile.version'
az aro list-upgrades -g ${RG} -n ${CLUSTER} -o table
az aro update -g ${RG} -n ${CLUSTER} --refresh-cluster
```
