---
name: remote-run-ssh
description: Run CVlization examples on the `ssh l1` GPU host by copying only the needed example directory plus the shared `cvlization/` package into `/tmp`, then launching the example’s Docker scripts. Use when this capability is needed.
metadata:
  author: kungfuai
---
