...
### Example 4: Creating a Spec Validation Skill

User request:
```
Create a skill that validates spec quality before proceeding
```

You would:
1. Read the documentation files
2. Define scope with user:
   - What to validate? (EARS format, traceability, completeness)
   - When to validate? (After each phase or on-demand)
   - What level of strictness? (Warnings vs errors)
3. Review existing spec skills for patterns:
   - See requirements-skill/VALIDATION.md for quality checks
   - See design-skill for traceability patterns
   - See tasks-skill for completeness checks
4. Create structure:
   ```bash
   mkdir -p .claude/skills/spec-validation
   ```
5. Write SKILL.md with:
   ```yaml
   ---
   name: Spec Quality Validation
   description: Validate specification quality across all phases. Use when reviewing specs, checking EARS format, verifying traceability, or ensuring completeness before phase transitions.
   allowed-tools: Read, Grep
   ---
   ```
6. Add validation workflow:
   - Load spec document
   - Check format (EARS keywords, SHALL usage)
   - Verify traceability (REQ references present)
   - Assess completeness (all sections filled)
   - Generate validation report
7. Create supporting files:
   - `CHECKLIST.md` - Complete validation checklist
   - `EXAMPLES.md` - Good vs bad spec examples
   - `FIXES.md` - How to fix common issues
8. Test with sample specs from examples/ directory

### Example 5: Extending an Existing Skill
