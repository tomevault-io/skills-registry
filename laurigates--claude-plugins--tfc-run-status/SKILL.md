---
name: tfc-run-status
description: Quick status check for Terraform Cloud runs showing status, resource changes, timestamps, and available actions. Use when monitoring run progress or checking if a run can be applied/canceled. Requires TFE_TOKEN environment variable. Use when this capability is needed.
metadata:
  author: laurigates
---
