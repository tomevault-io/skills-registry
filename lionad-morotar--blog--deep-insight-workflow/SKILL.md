...
description: 资料深度分析工作流，帮助用户从上传的资料（文档、网页、视频、图片等）中提取深度洞见、发现矛盾、构建框架、识别风险。当用户需要深度分析资料、进行尽职调查、撰写综述、提取可执行计划时触发。包含10个分析模块，支持依赖驱动的灵活组合和 reAct 循环。 Use when this capability is needed.
metadata:
