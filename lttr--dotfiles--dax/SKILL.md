---
name: dax
description: This skill should be used when creating shell scripts, CLI tools, automation scripts, or any task that needs to run shell commands programmatically. Trigger when user asks to write a script, automate a task, create a CLI tool, process files in batch, or build any executable that runs shell commands. Also trigger when user mentions "dax", "deno script", or "write me a script". Deno with @david/dax is the default scripting stack on this machine - prefer it over bash scripts for anything beyond trivial one-liners. Use when this capability is needed.
metadata:
  author: lttr
---
