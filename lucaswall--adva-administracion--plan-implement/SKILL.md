...
**Rationale:** Pure task/file counts miss complexity. Five surgical fixes (5×S=5 points) don't justify workers even across 7 files, but four substantial features (4×L=16 points) clearly do. The effort score captures this. Calibrated from real iterations: a batch of 7 mixed M/L tasks (e.g., new service module + 3 routes + 2 parsers) → workers justified; a batch of 3 small fixes to `parser.ts` → worker overhead exceeded the work; a fix plan of 5 S-sized CUIT/validation tweaks → single-agent was correct.
...
2. Identify the primary layer: types/schema → service/business logic → API routes → utilities
...
1. Group by functional domain: "CUIT validation," "spreadsheet parsing," "Gemini integration"
