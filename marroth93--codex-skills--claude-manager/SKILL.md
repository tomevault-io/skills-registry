---
name: claude-manager
description: Use when Codex should implement tasks but Claude Code should review and approve plans and final changes. Trigger on requests like "review with claude", "have claude approve this", or "codex implementation with claude review". Accepts direct tasks or plan-file based execution.
metadata:
  author: marroth93
---
...
2. **Dual approval required** — Claude must approve the plan and the final changes.
...
5. **Claude reviews** — Claude does review/approval only; it does not modify files.
6. **Iterate to consensus** — Keep looping until Claude approves or the user changes direction.
...
1. Keep each Claude review payload focused on one decision.
...
6. Store a rolling summary in `.claude-manager/session-handoff.md` after each major step.
...
```bash
cat <<'EOF' | "$CLAUDE_MANAGER_HOME/scripts/claude-task.sh" review-plan
## Implementation Plan
...
[Include key snippets Claude needs for plan validation]
EOF
```
...
- Keep `.claude-manager/session-handoff.md` current after each completed step.
...
```bash
cat <<'EOF' | "$CLAUDE_MANAGER_HOME/scripts/claude-task.sh" review-changes
## Changes Review
...
- `VERDICT: REJECT` → Codex fixes issues, then re-submit to Claude.
...
2. Summarize issues Claude flagged and how they were resolved.
...
### Plan File Utilities
```bash
"$CLAUDE_MANAGER_HOME/scripts/plan-utils.sh" validate <plan_file.md>
"$CLAUDE_MANAGER_HOME/scripts/plan-utils.sh" status <plan_file.md>
"$CLAUDE_MANAGER_HOME/scripts/plan-utils.sh" next <plan_file.md>
"$CLAUDE_MANAGER_HOME/scripts/plan-utils.sh" complete "Task Name" <plan_file.md>
"$CLAUDE_MANAGER_HOME/scripts/plan-utils.sh" list <plan_file.md>
```
...
1. Run `"$CLAUDE_MANAGER_HOME/scripts/claude-task.sh" --check`.
2. Repair Claude Code CLI/auth.
...
3. Prefer smaller model/context where acceptable.
...
2. Update `.claude-manager/session-handoff.md` with current status.
3. Run:
...
5. Re-run Claude review if strategy changed.

### Interrupted Session / Crash / Usage Cutoff

Recovery sequence:
1. `git status`
2. `"$CLAUDE_MANAGER_HOME/scripts/plan-utils.sh" status <plan_file.md>`
3. Read `.claude-manager/session-handoff.md`
...
5. Record degraded-mode decision in `.claude-manager/session-handoff.md`.
