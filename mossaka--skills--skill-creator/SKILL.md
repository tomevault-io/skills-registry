---
name: skill-creator
description: Guide for creating effective agent skills. This skill should be used when users want to create a new skill (or update an existing skill) that extends agent capabilities with specialized knowledge, workflows, or tool integrations. Use when this capability is needed.
metadata:
  author: mossaka
---
...
```bash
scripts/init_skill.py <skill-name> --path ~/.claude/skills/<skill-name>
```

Example:
```bash
scripts/init_skill.py pdf-editor --path ~/.claude/skills/pdf-editor
```
