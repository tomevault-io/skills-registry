...
5. **Offer to continue with ralph** to break into executable tasks

**Important:** Do NOT start implementing. Create the PRD, then optionally hand off to ralph.
...
## After Saving: Offer Ralph Handoff
...
Ready to break this into executable tasks?
1. Yes, continue with ralph (creates progress.txt with tasks)
2. No, I'll review the PRD first
```

**If user chooses 1:** Invoke the ralph skill with the PRD path:
- Pass the PRD file path to ralph
- Ralph will read the user stories and convert them to tasks
- No need for ralph to ask clarifying questions - the PRD has the answers

This creates a seamless flow: `/prd` → `/ralph` → `/build-feature`
