---
name: committer
description: Git コミットの作成を支援する。以下の場合に発動: (1) ユーザーが /commit と入力した時、(2) コード変更が完了しコミットが適切と判断した時、(3) ユーザーがコミットを依頼した時。変更内容を分析し、適切なコミットメッセージを生成してコミットを作成する。 Use when this capability is needed.
metadata:
  author: muryoimpl
---
...
変更したファイルが隠しファイルの場合、追加するかどうかを確認し、OK の場合は `git add <files> -f` で追加する
