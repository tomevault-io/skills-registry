...
**Note:** This skill requires the Rube MCP server to be configured for HackerNews search and Slack integration features. Without Rube MCP, steps 5 (learning resources curation) and 7 (Slack delivery) will be skipped, but the core analysis and report generation will still work.
...
5. **Search for Learning Resources** (Requires Rube MCP - Optional)

   If Rube MCP is configured, search HackerNews for articles related to each improvement area:
   ...
   If Rube MCP is not available, use WebSearch as an alternative or skip this step and deliver the report without curated resources.
...
7. **Send Report to Slack DMs** (Requires Rube MCP - Optional)

   If Rube MCP is configured, send the complete report to the user's own Slack DMs:
   ...
   If Rube MCP is not configured, skip this step and present the complete report directly in the CLI output for the user to save or share manually.
