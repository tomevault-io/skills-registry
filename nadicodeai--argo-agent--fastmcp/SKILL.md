...
- smoke-test a server with the FastMCP CLI before wiring it into Argo or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Argo. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.argo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.argo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Argo integration, either:

- configure the server in `~/.argo/config.yaml` using the `native-mcp` skill, or
...
### Argo cannot see the deployed server

The server-building part may be correct while the Argo config is not. Load the `native-mcp` skill and configure the server in `~/.argo/config.yaml`, then restart Argo.
...
---
> Source: [nadicodeai/argo-agent](https://github.com/nadicodeai/argo-agent) — distributed by [TomeVault](https://tomevault.io).
