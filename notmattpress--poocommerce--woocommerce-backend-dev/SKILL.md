...
description: Add or modify PooCommerce backend PHP code following project conventions. Use when creating new classes, methods, hooks, or modifying existing backend code. **MUST be invoked before writing any PHP unit tests.** Use when this capability is needed.
metadata:
  author: notmattpress
---
...
- Use PSR-4 autoloading with `Automattic\PooCommerce` namespace
...
- Read the `$version` property in `includes/class-poocommerce.php` **on the trunk branch**
