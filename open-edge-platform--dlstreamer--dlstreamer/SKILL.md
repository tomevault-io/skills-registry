...
description: Build new DL Streamer video-analytics applications (Python, C, C++ or GStreamer command line). Use when: user describes a vision AI pipeline, wants to create a new sample app, combine elements from existing samples, add detection/classification/VLM/tracking/alerts/recording to a video pipeline, or create custom GStreamer elements in Python or C++. Translates natural-language pipeline descriptions into working DL Streamer code using established design patterns. Use when this capability is needed.
metadata:
...
---
> Source: [open-edge-platform/dlstreamer](https://github.com/open-edge-platform/dlstreamer) — distributed by [TomeVault](https://tomevault.io).
