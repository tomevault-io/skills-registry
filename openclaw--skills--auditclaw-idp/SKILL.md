---
name: auditclaw-idp
description: Identity provider compliance checks for auditclaw-grc. 8 read-only checks across Google Workspace (MFA, admin audit, inactive users, passwords) and Okta (MFA, password policy, inactive users, session policy). Use when this capability is needed.
metadata:
  author: openclaw
---
# AuditClaw IDP
