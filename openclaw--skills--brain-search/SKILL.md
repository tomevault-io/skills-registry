---
name: brain-search
description: **You MUST actually execute every curl command using your shell/exec tool.** Read the real HTTP response. NEVER generate a fake response, placeholder ID, or simulated output. If the API call fails, report the actual error to Boss Man. If you cannot execute shell commands right now, say so — do not pretend you ran them. Use when this capability is needed.
metadata:
  author: openclaw
---
# Skill: brain-search
