...
description: Generate Catch2 unit tests following ORE Studio conventions. Use when this capability is needed.
metadata:
  author: orestudio
---
...
Protocol tests verify binary serialize/deserialize roundtrips. They require no infrastructure and are fast to run.
...
Generator tests verify that synthetic data generators produce valid, populated entities.
...
-   `database_helper` - Original pattern. Some components (DQ, IAM) pass context to the repository constructor.
-   `scoped_database_helper` - Newer pattern with per-test tenant isolation. No table truncation needed. Used by Refdata.

-   ****Write Operations**** (`database_helper` style)
...
-   ****Write Operations**** (`scoped_database_helper` style)
...
2.  ****Tenant Isolation****: Use `scoped_database_helper` for tenant-isolated tests (no table truncation needed). Use `database_helper` for components that pass context to the repository constructor.
3.  ****Descriptive Names****: Test names should clearly describe what is being tested
4.  ****Assertions****: Use `CHECK` for non-fatal assertions, `REQUIRE` for fatal ones
5.  ****Setup/Teardown****: Use database\_helper which handles setup/teardown automatically
...
10. ****String containment****: Use C++23's `std::string::contains()` method instead of `find() !` std::string::npos= for substring checks. It's cleaner and more readable:
...
See [Unit Test Database Integration Guide](../../../projects/ores.testing/modeling/database_integration.md) to understand how Unit Tests integrate with the database.

---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/orestudio) — claim your Tome and manage your conversions.
