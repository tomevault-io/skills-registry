---
name: agent-eval
description: 编码代理（Antigravity、Aider、Codex等）在自定义任务上的直接比较，包含通过率、成本、时间和一致性指标 Use when this capability is needed.
metadata:
  author: pampam666
---
...
* 在你自己的代码库上比较编码代理（Antigravity、Aider、Codex 等）
* 在采用新工具或模型之前衡量代理性能
* 当代理更新其模型或工具时运行回归检查
* 为团队做出数据支持的代理选择决策
...
```bash
agent-eval run --task tasks/add-retry-logic.yaml --agent antigravity-code --agent aider --runs 3
```
...
├──────────────┼───────────┼────────┼────────┼─────────────┤
│ antigravity-code  │ 3/3       │ $0.12  │ 45s    │ 100%        │
...
---
> Source: [pampam666/awesome-website-test](https://github.com/pampam666/awesome-website-test) — distributed by [TomeVault](https://tomevault.io).
