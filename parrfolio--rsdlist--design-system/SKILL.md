...
A clean, elevated SaaS design system built on **shadcn/ui**, **Tailwind CSS v4**, and **React (Vite)**. Every interface generated — whether a client demo, internal tool, or quick prototype — must follow these rules to maintain a consistent, professional visual identity across the project.
...
2. **Dark mode** — Should the interface support dark mode, or is it light mode only? (Determines whether to implement `.dark` class overrides and use `bg-neutral-0` for surfaces.)
...
- **Tailwind CSS v4** — Use the theme file at `references/theme.css` as brand overrides
- **Radix UI** — Unstyled, accessible component primitives (foundation for shadcn/ui). Provides keyboard navigation, focus management, and ARIA attributes out of the box
- **shadcn/ui** — Built on top of Radix UI primitives. Initialize a standard shadcn/ui project first (`shadcn init`), then layer brand tokens from `references/theme.css` on top. The shadcn semantic layer (`bg-background`, `text-foreground`, `bg-primary`, `border-border`, etc.) is the base — the theme.css adds brand colors, typography, shadows, and spacing on top of it, not as a replacement. Use shadcn components directly. Do NOT create custom base components that duplicate shadcn functionality
...
The component stack follows a layered architecture:

```
Radix UI primitives (accessibility, keyboard nav, ARIA — unstyled)
  └── shadcn/ui components (Tailwind styling, variants, dark mode)
       └── Brand-themed components (design tokens, custom properties)
```

- **Radix UI** is the building block layer — never re-implement what Radix provides
- **shadcn/ui** is the styled component layer — always prefer these over custom builds
- **Brand theming** is applied via CSS custom properties and Tailwind config — never hard-code colors or spacing
- `@radix-ui/*` packages in `package.json` are expected dependencies, not separate installations
- Never install competing UI primitive libraries (MUI, Chakra, Mantine, Ant Design)
...
## Design Mock vs. Design System Tokens

When a specific design mock is provided alongside this design system, the mock's visual decisions take priority for that screen or component. The design system is a tool for consistency and efficiency — not a constraint that overrides intentional design choices. When a designer specifies a value, it represents an intentional choice. The system adapts to serve the design, not the other way around.

### Priority Rule

Use design system tokens to implement the mock's values wherever possible. When a mock shows a value that doesn't match an available token, use the mock's exact value and flag the gap.

**Example:** The design shows `24px` padding, but the closest tokens are `--spacing-md: 16px` and `--spacing-lg: 32px`. Use the exact `24px` from the design. Flag the missing token for the design system team to evaluate.

### Conflict Resolution

| Situation                                              | Action                                                                      |
| ------------------------------------------------------ | --------------------------------------------------------------------------- |
| **Token exists and matches the design**                | Use the token. This is the ideal case.                                      |
| **Token exists but the design uses a different value** | Use the design's value directly. Flag the discrepancy.                      |
| **No token exists for the design's value**             | Use the raw value. Flag it as a candidate for a new token.                  |
| **Design is systematically different from tokens**     | Recommend updating the token definitions to match the new design direction. |

### Documentation Requirement

When a mock-vs-token conflict is resolved, document:

- **Design reference** — which screen or component the conflict appeared in
- **Conflicting values** — what the token provides vs. what the design specifies
- **Resolution** — how it was resolved (used mock value / updated token / flagged for review)
- **Token recommendation** — whether a new token should be created or an existing one updated
...
- [ ] If dark mode was requested: `.dark` class applied, surfaces use `bg-neutral-0` (not `bg-white`)
...
---
> Source: [parrfolio/RSDList](https://github.com/parrfolio/RSDList) — distributed by [TomeVault](https://tomevault.io).
