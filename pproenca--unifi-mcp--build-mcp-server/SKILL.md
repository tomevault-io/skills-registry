...
Each phase narrows the design space. After Phase 5, hand off to the appropriate scaffold reference or companion skill.

## Prerequisites

- **TypeScript path:** Node.js >= 18, npm
- **Python path:** Python >= 3.10, pip
- **Testing:** `npx` (ships with npm) — used by `scripts/test-server.sh` and the MCP Inspector
...
After scaffolding, verify the server responds correctly:

```bash
# Start the server first (e.g. npx tsx src/server.ts or python server.py)
# Then smoke-test it:
bash scripts/test-server.sh http://localhost:3000/mcp
```

The test script calls `tools/list` via the MCP Inspector and checks the server returns at least one tool. If this passes, the server is wired correctly.
...
- [`references/remote-http-scaffold.md`](references/remote-http-scaffold.md) — Minimal remote server in TS SDK and FastMCP
- [`references/deploy-cloudflare-workers.md`](references/deploy-cloudflare-workers.md) — Fastest deploy path (Workers-native scaffold)
- [`references/tool-design.md`](references/tool-design.md) — Writing tool descriptions and schemas Claude understands well
- [`references/auth.md`](references/auth.md) — OAuth, CIMD, DCR, token storage patterns
- [`references/resources-and-prompts.md`](references/resources-and-prompts.md) — The two non-tool primitives
- [`references/elicitation.md`](references/elicitation.md) — Spec-native user input mid-tool (capability check + fallback)
- [`references/server-capabilities.md`](references/server-capabilities.md) — Instructions, sampling, roots, logging, progress, cancellation
- [`references/versions.md`](references/versions.md) — Version-sensitive claims ledger (check when updating)

---
> Source: [pproenca/unifi-mcp](https://github.com/pproenca/unifi-mcp) — distributed by [TomeVault](https://tomevault.io).
