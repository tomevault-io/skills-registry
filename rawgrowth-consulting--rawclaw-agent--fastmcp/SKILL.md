...
- smoke-test a server with the FastMCP CLI before wiring it into Rawclaw or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Rawclaw. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.rawclaw/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.rawclaw/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Rawclaw integration, either:

- configure the server in `~/.rawclaw/config.yaml` using the `native-mcp` skill, or
...
### Rawclaw cannot see the deployed server

The server-building part may be correct while the Rawclaw config is not. Load the `native-mcp` skill and configure the server in `~/.rawclaw/config.yaml`, then restart Rawclaw.
...
---
> Source: [Rawgrowth-Consulting/rawclaw-agent](https://github.com/Rawgrowth-Consulting/rawclaw-agent) — distributed by [TomeVault](https://tomevault.io).
