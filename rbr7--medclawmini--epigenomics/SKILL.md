...
- RNA-seq differential expression -> Use `rnaseq-deseq2`
- Variant calling/annotation from VCF -> Use `variant-analysis`
- Gene enrichment analysis -> Use `gene-enrichment`
- Protein structure analysis -> Use `protein-structure-retrieval`
...
# MedToolkit (for annotation)
from medtoolkit import MedToolkit
```
...
Q: What type of epigenomics data?
  ...
  ANNOTATION  -> Phase 6 (MedToolkit Annotation)
...
### Phase 6: MedToolkit Annotation Integration

Use MedToolkit tools for biological annotation of epigenomic findings.
...
def annotate_genes_with_medtoolkit(gene_list, tu):
    """Annotate a list of genes using MedToolkit tools.
...
## MedToolkit Tool Parameter Reference
...
- Use MedToolkit Ensembl tools to build minimal annotation
...
- **API rate limits**: MedToolkit annotation limited to ~20 genes per batch
...
7. **MedToolkit annotation** - Ensembl, SCREEN, ChIPAtlas, JASPAR, ENCODE for biological context

**Core packages**: pandas, numpy, scipy, statsmodels, pysam, gseapy
**MedToolkit tools**: 25+ tools across Ensembl, SCREEN, ENCODE, ChIPAtlas, JASPAR, ReMap, RegulomeDB, 4DN
...
---
> Source: [rbr7/MedClawMini](https://github.com/rbr7/MedClawMini) — distributed by [TomeVault](https://tomevault.io).
