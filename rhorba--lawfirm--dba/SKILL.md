...
Current migration version: **V62** — next migration must be **V63**.
...
2. **Naming**: `V{next}__short_description.sql` — current latest is V62, so next is **V63**
...
### Current Schema (V1–V62)
**Auth & Users:**
- `users` — with soft-delete, `user_profiles` (extended info), `user_roles`, `user_groups`
- `roles`, `permissions`, `role_permissions` — 37+ permissions seeded (USER_*, ROLE_*, CASE_*, CLIENT_*, LAWYER_*, FINANCIAL_*, INVOICE_*)
- `groups`, `group_roles`, `group_permissions` — group-based RBAC; new users auto-join "Default Users"
- `audit_logs` — request/response + entity trail with IP and metadata

**Legal Domain:**
- `cases` — dossier with status workflows, bilingual reference data
- `case_types`, `case_categories`, `case_statuses`, `case_type_statuses` — reference/lookup tables
- `case_sequences`, `case_templates` — numbering and templates
- `case_lawyers` — M:N between cases and lawyers
- `lawyers` — full CRUD, soft-delete
- `clients` — CIN uniqueness, linked to cases
- `tribunals` — bilingual (French/Arabic) court reference data

**Financial:**
- `financial_transactions` — ledger entries, per-case tracking
- `invoices`, `invoice_items` — full invoice lifecycle; PAID status auto-syncs to transactions
...
---
> Source: [rhorba/LawFirm](https://github.com/rhorba/LawFirm) — distributed by [TomeVault](https://tomevault.io).
