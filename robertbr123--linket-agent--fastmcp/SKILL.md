...
- smoke-test a server with the FastMCP CLI before wiring it into Linket or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Linket. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.linket/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.linket/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Linket integration, either:

- configure the server in `~/.linket/config.yaml` using the `native-mcp` skill, or
...
### Linket cannot see the deployed server

The server-building part may be correct while the Linket config is not. Load the `native-mcp` skill and configure the server in `~/.linket/config.yaml`, then restart Linket.
...
---
> Source: [robertbr123/Linket-Agent](https://github.com/robertbr123/Linket-Agent) — distributed by [TomeVault](https://tomevault.io).
