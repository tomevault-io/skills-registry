---
name: git-workflow
description: 标准化的代码暂存、提交（包含 GPG 签名）和推送流程。当需要使用约定式提交（Conventional Commits）并处理交互式 GPG 验证时使用。 Use when this capability is needed.
metadata:
...
3. **强制验证 (Pre-commit Validation)**：
   - **配置有效性**：运行 `nix flake check`。如果失败，停止提交并解决问题。
   - **代码整洁**：运行 `nix fmt`。如果格式化导致更改，请重新暂存这些更改。
4. **生成提交**：
   - 确定合适的类型（如 `feat`, `fix`）。
   - 编写简短的摘要（最多 50 个字符）。
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/Ruixi-rebirth) — claim your Tome and manage your conversions.
