---
name: agent-knowledge-framework
description: textual TUI 异步数据加载完整模式：@work(thread=True) 后台执行阻塞 IO → call_from_thread 回主线程渲染 → LoadingIndicator 过渡 → exclusive group 防抖 → dict 缓存 + stale check 防竞态 Use when this capability is needed.
metadata:
  author: st1page
---
