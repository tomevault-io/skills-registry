...
description: MUST be used for Vue.js tasks. Strongly recommends Composition API with `<script setup>` and TypeScript as the standard approach. Covers Vue 3, SSR, Volar, vue-tsc, and shared state patterns. Load for any Vue, .vue files, Vue Router, or Vite with Vue work. For Nuxt projects, pair this with the nuxt skill and let Nuxt-specific guidance override when they conflict. Use when this capability is needed.
metadata:
...
- If the project is Nuxt-based, load the `nuxt` skill and treat Nuxt conventions as the higher-priority framework layer for routing, data fetching, SSR boundaries, and shared app state.

### 1.1 Core references (load based on task scope)

- For non-trivial features (new components, state redesign, multi-file changes), load all four core references before coding:
  - `references/reactivity.md`
  - `references/sfc.md`
  - `references/component-data-flow.md`
  - `references/composables.md`
- For small, focused changes (prop fix, template tweak, single-file edit), load only the reference that applies to the change.
- When in doubt, load at least `references/reactivity.md` and `references/sfc.md`.
...
- Prefer a feature folder layout (`components/<feature>/...`, `composables/use<Feature>.ts`) when adding more than one component. In Nuxt projects, nest feature folders inside Nuxt's auto-import directories (e.g., `components/<feature>/`) so auto-imports continue to work.

## 2) Apply essential Vue foundations (required)

These are essential, must-know foundations. Apply them using the core references loaded in section `1.1` (scoped to task complexity).
...
- Core references matching the task scope were read and applied.
