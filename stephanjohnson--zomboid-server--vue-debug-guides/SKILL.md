...
description: Use when diagnosing Vue 3 runtime problems, warnings, hydration mismatches, async failures, or reactivity bugs. Provides a symptom-first debugging workflow and targeted reference guides.
metadata:
...
Use this skill when the task is debugging or fixing broken Vue behavior. For general implementation guidance and prevention patterns, use `vue-best-practices` first.

## When to Use

- Runtime errors or warnings in Vue components
- Reactivity that stops updating or updates unexpectedly
- Watcher, computed, lifecycle, or composable bugs
- SSR or hydration mismatches
- Template compilation or rendering failures

## Debug Workflow

1. Classify the symptom before changing code.
2. Reduce the bug to the smallest failing reactive boundary, component boundary, or SSR boundary.
3. Load only the reference group that matches the failure mode.
4. Fix the root cause, not the visible symptom.
5. Re-run the reproduction path and confirm the failure is actually gone.

## Required approach

- Start by deciding whether the issue is reactivity, rendering, events, lifecycle, SSR, or typing.
- Prefer a minimal reproduction path over broad refactors.
- Avoid stacking multiple speculative fixes in one pass.
- If the app is Nuxt-based, also check whether the bug is really a client/server boundary issue and load the `nuxt` skill.

## Category Quick Reference

Classify the symptom first, then scan the matching category in the Reference Index below:

| Symptom Area | Category |
|---|---|
| State not updating, proxy issues, reactive unwrapping | Reactivity |
| Computed getter bugs, stale values, mutations | Computed |
| Watcher not firing, stale data, memory leaks | Watchers |
| Component not found, ref access, naming | Components |
| Props/emits errors, double firing, scope issues | Props & Emits |
| Template compilation, v-if/v-for, rendering | Templates |
| Template ref null, wrong order, v-if issues | Template Refs |
| v-model, form values, input sync | Forms & v-model |
| Event modifier order, keyboard shortcuts | Events & Modifiers |
| Memory leaks, DOM timing, SSR lifecycle | Lifecycle |
| Slot scope, forwarding, compilation errors | Slots |
| Provide/inject reactivity, debugging | Provide/Inject |
| Fallthrough attrs, event merging | Attrs |
| Composable call location, reactivity loss | Composables |
| Async context, defineExpose timing | Composition API |
| Animation not triggering, performance | Animation |
| Type errors, withDefaults, template types | TypeScript |
| Async component loading, KeepAlive refs | Async Components |
| Render function vnodes, h() slots | Render Functions |
| KeepAlive double mount, memory | KeepAlive |
| Transition timing, move animations | Transitions |
| Teleport target, SSR, scoped styles | Teleport |
| Suspense error handling, SSR | Suspense |
| Hydration mismatch, cross-request state | SSR |
| Unnecessary re-renders, computed stability | Performance |
| Scoped CSS, script setup, Tailwind | SFC |
| Plugin install order, global properties | Plugins |
| App config timing, mount chaining | App Configuration |
...
- The failure was categorized correctly before code changed.
- Only the relevant debug references were loaded.
- The fix addresses the root cause instead of suppressing the symptom.
- The original reproduction path now passes.
- Related SSR or Nuxt boundary issues were checked when applicable.
