---
name: coding-agent
description: Run Codex CLI, Claude Code, OpenCode, Pi Coding Agent, or iFlow via background process for programmatic control. Use when this capability is needed.
metadata:
  author: tclawde
---
...
- `-y, --yolo`: Automatically accept all actions (no approval needed).
- `-s, --sandbox`: Run in sandboxed mode.
- `-m, --model`: Specify model to use.
- `-o, --output-file`: Save execution info to file (non-interactive only).
- `--all-files`: Include ALL files in context.
- `--max-turns`: Limit model calls before terminating.
...
# With sandbox and output file
bash workdir:~/project background:true command:"iflow -s -o /tmp/result.json -p \"Review PR #42\""

# Specify model
bash workdir:~/project background:true command:"iflow -m claude-sonnet-4 -p \"Refactor the utils\""
```
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/tclawde) — claim your Tome and manage your conversions.
