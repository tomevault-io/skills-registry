...
**Trigger Keywords**: `async`, `await`, `asyncio`, `gather`, `concurrent`, `blocking`, `slow`, `timeout`, `rate limit`

**Use for this project (AI Sales Agent)**:
| Week | Task | Pattern Needed |
|------|------|----------------|
| 1.4 | AsyncEngine + AsyncSession | Pattern 6: Async Context Managers |
| 1.5 | FastAPI async routes | Pattern 1: Basic Async/Await |
| 1.7 | Async seeding script | Pattern 2: gather() for bulk inserts |
| 2.1 | Embedding API calls | Pattern 9: Semaphore rate limiting |
| 3.2 | Async LangGraph tools | Pattern 1 + error handling |

**General use cases**:
- FastAPI route handlers (always use `async def`)
- Database queries with SQLAlchemy async
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/thaig2pro) — claim your Tome and manage your conversions.
