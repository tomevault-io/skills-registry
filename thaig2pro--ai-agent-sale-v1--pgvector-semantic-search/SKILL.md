...
| Week | Task | What You Need |
|------|------|---------------|
| 1.3 | Docker DB setup | Enable pgvector extension |
| 1.4 | SQLModel schema | Add `embedding` column as `halfvec(N)` |
| 2.1 | Embedding pipeline | Store embeddings from Gemini/Mixedbread |
| 2.2 | Async Vector Search | Query with cosine distance `<=>` |
| 2.5 | Hybrid Search | Combine vector + keyword (optional) |

**For SME scale (< 1M products)**: Use the Golden Path below. Skip partitioning, binary quantization, and advanced tuning.
