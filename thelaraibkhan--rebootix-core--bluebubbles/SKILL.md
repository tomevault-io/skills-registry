---
name: bluebubbles
description: Build or update the BlueBubbles external channel plugin for Rebootix (extension package, REST send/probe, webhook inbound). Use when this capability is needed.
metadata:
...
- Route into core reply pipeline via the plugin runtime (`api.runtime`) and `rebootix/plugin-sdk` helpers.
