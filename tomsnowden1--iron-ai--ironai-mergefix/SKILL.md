...
description: Resolve Git merge and rebase conflicts safely for the IronAI repository. Use when `git status` shows unmerged paths, conflict markers (`<<<<<<<`, `=======`, `>>>>>>>`) are present, `git pull --rebase` fails with conflicts, or README/docs workflow sections diverge. Use when this capability is needed.
metadata:
  author: tomsnowden1
---
