...
description: Terraform の実装/運用の「やり方」を標準化するためのガイドライン集。構成/モジュール設計/命名/環境分離/backend/state/upgrade/import/moved/レビュー観点、および `terraform fmt/validate/plan/apply` と周辺ツール（tflint、tfsec、checkov、terraform-docs、infracost、pre-commit、CI）の使い方の相談に使う。 Use when this capability is needed.
metadata:
  author: tr-yasuda
---
