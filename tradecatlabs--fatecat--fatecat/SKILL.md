---
name: fatecat
description: FateCat 执行型测算基础设施 skill：安装并校验当前仓库，执行生产化 capability，输出 JSON / Markdown，启动 Web / API / Telegram 交付层，并在发布前运行仓库卫生与生产就绪门禁。Use when 用户要求测算、排盘、生成报告、启动 Web/API/Bot、验收 skill、导出 bundle、检查生产可用性。 Use when this capability is needed.
metadata:
  author: tradecatlabs
---
