...
description: One-off image generation (prompt -> image file) using Transloadit via the `transloadit` CLI. Prefer `image generate` for text-only and input-guided generation, and download outputs locally via `-o`. Use when this capability is needed.
metadata:
  author: transloadit
---
