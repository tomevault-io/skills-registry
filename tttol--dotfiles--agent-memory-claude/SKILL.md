...
**Location:** `.claude/skills/agent-memory-claude/memories/`
...
```bash
# 1. List categories
ls .claude/skills/agent-memory-claude/memories/

# 2. View all summaries
rg "^summary:" .claude/skills/agent-memory-claude/memories/ --no-ignore --hidden

# 3. Search summaries for keyword
rg "^summary:.*keyword" .claude/skills/agent-memory-claude/memories/ --no-ignore --hidden -i

# 4. Search by tag
rg "^tags:.*keyword" .claude/skills/agent-memory-claude/memories/ --no-ignore --hidden -i

# 5. Full-text search (when summary search isn't enough)
rg "keyword" .claude/skills/agent-memory-claude/memories/ --no-ignore --hidden -i
...
```bash
mkdir -p .claude/skills/agent-memory-claude/memories/category-name/
# Note: Check if file exists before writing to avoid accidental overwrites
cat > .claude/skills/agent-memory-claude/memories/category-name/filename.md << 'EOF'
---
...
- **Delete**: Remove memories that are no longer relevant
  ```bash
  trash .claude/skills/agent-memory-claude/memories/category-name/filename.md
  # Remove empty category folders
  rmdir .claude/skills/agent-memory-claude/memories/category-name/ 2>/dev/null || true
  ```
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/tttol) — claim your Tome and manage your conversions.
