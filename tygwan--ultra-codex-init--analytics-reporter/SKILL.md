...
description: Agent/Skill 사용 통계 및 성과 리포트 생성 전문가. CLI 차트로 시각화하고 인사이트를 제공합니다. "통계", "사용량", "analytics", "성과", "리포트", "메트릭", "분석", "usage", "metrics", "statistics", "report" 키워드에 반응. Use when this capability is needed.
metadata:
  author: tygwan
---
...
| `planning` | TodoWrite |
| `interaction` | ask the user, enter plan mode, exit plan mode |
...
cat .codex/analytics/metrics.jsonl | jq -s '
...
→ Run: .codex/scripts/analytics-visualizer.sh summary
```

### Full Report
```
"전체 리포트" / "full analytics report"
→ Run: .codex/scripts/analytics-visualizer.sh full
```

### Specific Views
```
"도구별 통계" → .codex/scripts/analytics-visualizer.sh tools
"에러 분포" → .codex/scripts/analytics-visualizer.sh errors
"시간대별" → .codex/scripts/analytics-visualizer.sh activity
"에이전트 사용" → .codex/scripts/analytics-visualizer.sh agents
```

## Settings Reference

From `.codex/settings.json`:
...
- **Script**: `.codex/scripts/analytics-visualizer.sh` - CLI rendering
