---
name: workflow-lint-skill-v1-4-0
description: ワークフロー/スキル/README lint の技術仕様（rules/分割対応） Use when this capability is needed.
metadata:
  author: u-ss
---
...
| ファイル | カテゴリ | ルール数 |
|:---------|:---------|:--------:|
| `rules/00_meta.md` | バージョン整合・正本境界 | 3 |
| `rules/10_files.md` | 必須ファイル・エンコーディング | 5 |
| `rules/20_template.md` | テンプレート・役割境界 | 4 |
| `rules/30_sub_agents.md` | 子エージェント規約 | 4 |
| `rules/40_cross_ref.md` | クロスリファレンス | 4 |
| `rules/50_logging.md` | ログ統合 | 1 |
| `rules/60_readme.md` | README形式 | 5 |
| `rules/70_modes.md` | **modes/ 検証** ★v1.4.0 | 4 |

> 各ルールの詳細（Rule ID、重大度、判定条件、実装マッピング）は `rules/*.md` を参照。
...
| WL-VER-002 | ERROR | SKILL.mdとWORKFLOW.mdのH1バージョンが不一致 |
...
| WL-MODE-001 | `lint_modes()` | `prerequisite path '../SPEC.md' not found` |
| WL-MODE-002 | `lint_modes()` | `modes/ exists but SKILL.md has no modes/*.md references` |
| WL-MODE-003 | `lint_modes()` | `SKILL.md references 'modes/xxx.md' but file not found` |
| WL-MODE-004 | `lint_modes()` | `not referenced from SKILL.md mode refs` |
