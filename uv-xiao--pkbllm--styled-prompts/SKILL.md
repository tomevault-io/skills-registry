...
description: Convert per-page Content PROMPTs into design-complete Styled PROMPTs using a Markdown style brief, inferring the best layout per page during creation. Outputs $HUMAN_MATERIAL_PATH/slides/<deck>/prompts/styled/<deck>.md, ready for image/PDF/PPT generation. Use when this capability is needed.
metadata:
  author: uv-xiao
---
...
2. **Do not render anything** here (no images/PDF/PPTX, no render sanity-checks). Rendering belongs to `$uv-styled-artifacts`.
...
- **Next (render PDF / image-PPTX)**: run `$uv-styled-artifacts`.
  - PDF + image-PPTX:
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/uv-xiao) — claim your Tome and manage your conversions.
