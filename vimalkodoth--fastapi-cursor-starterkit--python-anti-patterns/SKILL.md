...
description: Common Python anti-patterns to avoid. Use as a checklist when reviewing code, before finalizing implementations, or when debugging issues that might stem from known bad practices. Use when this capability is needed.
metadata:
  author: vimalkodoth
---
...
```python
# GOOD (Pydantic v2: use model_validate, not from_orm)
@app.get("/users/{id}")
...
**Fix:** Use async-native libraries. In this project, if you must call a blocking function (e.g. sync RPC), run it in a thread pool: `await asyncio.to_thread(blocking_func, ...)` so the event loop stays responsive.
...
| Exposed ORM models | DTO/response schemas (Pydantic model_validate) |
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/vimalkodoth) — claim your Tome and manage your conversions.
