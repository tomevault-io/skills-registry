...
In this project use **PostgreSQL** with **SQLModel** and **AsyncSession**; connection pooling is configured in `backend/app/core/database.py`. Apply batching and query optimization to async operations; see `.cursor/rules/database-postgres.md`. Examples below use sqlite for a minimal illustration.
