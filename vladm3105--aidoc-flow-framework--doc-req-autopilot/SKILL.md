...
description: Automated REQ generation from SYS requirements - decomposes system requirements into atomic files with SPEC-Ready scoring Use when this capability is needed.
metadata:
  author: vladm3105
---
...
2. **Set** `document_type` in generated document frontmatter:
   ```yaml
   custom_fields:
     document_type: req-document    # NOT "template"
     artifact_type: REQ
     layer: 7
   ```

3. **Validation**: Generated documents MUST have `document_type: req-document`
