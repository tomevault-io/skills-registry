...
description: Automated SYS generation from ADR decisions - generates system requirements with REQ-Ready scoring Use when this capability is needed.
metadata:
  author: vladm3105
---
...
2. **Set** `document_type` in generated document frontmatter:
   ```yaml
   custom_fields:
     document_type: sys-document    # NOT "template"
     artifact_type: SYS
     layer: 6
   ```

3. **Validation**: Generated documents MUST have `document_type: sys-document`
