...
- Ignoring the boundary: Do not automate undefined or unsafe work.
- Accepting the rationalization: "CI will catch it" fails when the gate is not wired into CI or npm run check.
- Proceeding despite this red flag: Automation writes outside intended workspace or provider config scope.
- Letting the known failure mode happen: Pipeline passes locally but fails in CI due environment assumptions.
...
6. For SSH, server, provider, database, publish, or cleanup side effects, record the exact user-approved target, command family, blast radius, and restore path before enabling writes.
...
SSH/server/provider/database mutation? -> read-only diagnostics until exact scoped user approval exists
...
4a. Gate write mode behind scoped approval and target-boundary checks. Automation must not delete, reset, deploy, publish, rotate secrets, or mutate remote/server/database state merely because credentials are available.
...
- Automation treats SSH/cloud/provider credentials as permission to mutate live resources without an explicit approval ledger.
