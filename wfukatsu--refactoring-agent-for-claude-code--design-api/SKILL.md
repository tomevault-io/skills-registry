---
name: design-api
description: API設計エージェント - リファクタリング後のマイクロサービスAPI設計。REST/GraphQL/gRPC/AsyncAPIの仕様書、API Gateway設計、認証・認可設計を策定。/design-api [対象パス] で呼び出し。 Use when this capability is needed.
metadata:
  author: wfukatsu
---
...
**このステップ完了時に出力**: `reports/03_design/api-specifications/openapi/[service]-api.yaml` - 各サービスのOpenAPI仕様を生成
...
**このステップ完了時に出力**: `reports/03_design/api-specifications/graphql/[service]-schema.graphql` - 各サービスのGraphQLスキーマを生成
...
**このステップ完了時に出力**: `reports/03_design/api-specifications/grpc/[service].proto` - 各サービスのProtocol Buffers定義を生成
...
**このステップ完了時に出力**: `reports/03_design/api-specifications/asyncapi/[service]-events.yaml` - 各サービスのAsyncAPI仕様を生成
...
- **前提ファイルが存在しない場合** → エラーメッセージを表示し、不足ファイルを明示して `/design-microservices` の実行を提案
- **ターゲットアーキテクチャが不完全な場合** → 存在するサービスのみでAPI設計を進め、未確認箇所を `[要確認]` でマーク
- **特定のAPIプロトコルが不要な場合** → 該当ステップをスキップし、理由をレポートに記載（例: 内部サービスのみのためGraphQL不要）
- **セキュリティモデルと既存設計が競合する場合** → AskUserQuestion でユーザーに判断を委ねる
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/wfukatsu) — claim your Tome and manage your conversions.
