...
description: マイクロサービス設計エージェント - ターゲットアーキテクチャ、変換計画、運用計画の策定。/design-microservices [対象パス] で呼び出し。 Use when this capability is needed.
metadata:
  author: wfukatsu
---
...
- **前提ファイルが存在しない場合** → エラーメッセージを表示し、不足ファイルを明示して `/analyze-system` や `/map-domains` の実行を提案
- **ドメイン分析が不完全な場合** → 存在するドメインのみで設計を進め、未確認箇所を `[要確認]` でマーク
- **循環依存が検出された場合** → Sagaパターンまたはイベント駆動アーキテクチャへの変更を提案
- **サービス境界が競合する場合** → AskUserQuestion で判断を委ねる
