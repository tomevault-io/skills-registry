...
description: ScalarDB Analytics設計エージェント - ScalarDB Analyticsを使用した分析基盤の設計。Apache Sparkベースの分析クエリ、マルチDB横断分析を策定。/design-scalardb-analytics [対象パス] で呼び出し。 Use when this capability is needed.
metadata:
  author: wfukatsu
---
...
- `reports/03_design/scalardb-architecture.md`（必須 - `/design-scalardb` Phase完了後に生成）
- `reports/03_design/scalardb-schema.md`（オプション）
