...
### Debugging Frozen Containers

**How to get inside a container that has no shell (distroless) or is crashing?**

1. **Use `nsenter`:** Enter the namespace of the PID directly from the host.
...
2. **Ephemeral Debug Container:** Attach a new busybox sidecar to the *same* namespaces.

    ```bash
    docker run -it --rm --pid=container:<target_container> --net=container:<target_container> busybox sh
    ```

    *Why?* You can use networking tools (`netstat`, `curl`) from busybox as if you were *inside* the target container.

### CI/CD Layer Caching (GitHub Actions)

**Build usually takes 5 min? Make it 30s.**
