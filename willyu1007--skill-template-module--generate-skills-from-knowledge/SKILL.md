...
description: Turn one or more knowledge documents into a provider-agnostic Agent Skills bundle (SKILL.md + examples/templates), using a plan -> apply -> lint workflow. Use when this capability is needed.
metadata:
  author: willyu1007
---
...
- You SHOULD avoid hard-coded repository paths unless the target is explicitly a known repo-local convention (for example `dev-docs/`) with an agreed layout.
