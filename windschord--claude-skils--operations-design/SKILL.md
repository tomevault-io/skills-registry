...
description: 運用設計コンサルタントとして、対象業界の調査とヒアリングに基づきITIL 4・SRE・DevOpsベストプラクティスの運用設計書を作成する。新規サービスの運用設計、既存システムの運用改善、運用移管の準備に使用する。Do NOT use for ソフトウェアの機能設計（software-designingを使用すること）。 Use when this capability is needed.
metadata:
  author: windschord
---
...
| 入力 | 必須/任意 | 説明 |
|------|----------|------|
| サービス概要 | 必須 | サービスの目的、対象ユーザー、提供価値 |
| システム構成情報 | 必須 | アーキテクチャ、技術スタック、インフラパターン |
| 運用要件 | 必須 | 可用性目標、サービス提供時間、セキュリティ要件 |
| 既存ドキュメント | 任意 | システム設計書、要件定義書、既存運用設計書 |
| 業界情報 | 任意 | 対象業界、規制要件、コンプライアンス要件 |

### 出力（Outputs）

| 出力 | 形式 | 説明 |
|------|------|------|
| 運用設計書 | Markdown | 20セクションの包括的な運用設計ドキュメント |
| 会話ログ | Markdown | ヒアリング内容と決定事項の記録 |
...
```mermaid
flowchart TD
    A[0. 会話ログの初期化] --> B[0.5. インフラパターンの選択]
    B --> C[1. 業界トレンド調査]
    C --> D[2. サービス仕様のヒアリング]
    D --> E[3. 運用設計書の作成]
    E --> F[4. 各セクションでのユーザー確認]
    F --> G[5. 一貫性チェック]
    G --> H[6. 最終レビューと承認]

    A -.- A1[会話ログファイルの作成<br/>セッション情報の記録]
    B -.- B1[クラウドネイティブ/IaaS/オンプレミス<br/>適切なテンプレートの選択]
    C -.- C1[対象業界の調査<br/>調査結果を会話ログに記録]
    D -.- D1[既存ドキュメントの確認<br/>各質問と回答を会話ログに記録]
    E -.- E1[概要・SLO/SLI・運用体制<br/>各セクションの承認状況を記録]
    F -.- F1[内容の確認・修正・承認<br/>確認結果を会話ログに記録]
    G -.- G1[矛盾の検出・修正提案<br/>チェック結果を会話ログに記録]
    H -.- H1[全体の整合性確認<br/>最終的な決定事項を記録]
```
...
**詳細な質問項目**: `hearing_items/2-9_security_compliance_ja.md` を Readツールで参照し、プロジェクトの性質に応じて適切な質問を選択・実施。

**会話ログ記録**: `references/conversation_logging_ja.md` の形式で、回答内容を会話ログに記録。
