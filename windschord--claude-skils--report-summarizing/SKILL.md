...
description: 調査結果や分析レポートを4層構造（エグゼクティブサマリー→概要→詳細→Appendix）で整理し、意思決定者向けに構造化する。レポート作成、既存ドキュメントの再構成、経営層向け報告書の作成に使用する。Do NOT use for ナレッジベースへの情報保存（knowledge-baseを使用すること）。 Use when this capability is needed.
metadata:
  author: windschord
---
