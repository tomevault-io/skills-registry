---
name: task-planning
description: 設計書から実装タスクへの分解と管理書を作成・編集する。AIエージェント向けの具体的な実装指示やTDD手順を定義する。タスク計画フェーズのみを単独で実行する際に使用する。Do NOT use for SDDワークフロー全体の管理（sdd-documentationを使用すること）。 Use when this capability is needed.
metadata:
  author: windschord
---
