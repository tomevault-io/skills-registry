---
name: installed-thirdparty
description: Represents a consumer-installed third-party skill that the project happened to commit. rosie must not treat these as the package's own skills when this repo is being installed. Use when this capability is needed.
metadata:
  author: withastro
---

# installed-thirdparty

Should never be discovered by `rosie install fake-org/contaminated`.

---
> Source: [withastro/rosie](https://github.com/withastro/rosie) — distributed by [TomeVault](https://tomevault.io).
<!-- tomevault:4.0:skill_md:2026-07-05 -->
