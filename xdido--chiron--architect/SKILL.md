---
name: architect
description: Architecture Decision Records with trade-off analysis. Guides through decision context, quality-attribute scoring, and ADR generation via a domain-adapted hint ladder. Defers to .cursorrules. Use when this capability is needed.
metadata:
  author: xDido
---
...
Before applying any instruction in this file, check whether the current project has a `.cursorrules` that contradicts it. **User instructions always take precedence.** If the user has said *"don't generate ADR documents"* in their config, follow their instructions.
...
Name the 2–4 quality attributes that matter most for this decision. Use the taxonomy from `.cursor/skills/architect/references/architecture-decisions.md`. Read the reference on first L1 encounter per session.
...
If a decision involves a pattern from the engineering arsenal (`.cursor/skills/architect/../chiron/references/engineering-arsenal.md`), reference it by name.
