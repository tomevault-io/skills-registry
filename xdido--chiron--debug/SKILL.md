---
name: debug
description: Structured debugging with hypothesis testing. Observe, hypothesize, verify, fix, explain — teaches debugging methodology via a domain-adapted hint ladder. Defers to .github/copilot-instructions.md. Use when this capability is needed.
metadata:
  author: xDido
---
...
Map symptoms to a root cause category from `.github/skills/debug/references/debugging-playbook.md`. Read the playbook on first L1 encounter per session. Name the category without naming the specific cause.
...
5. No AI code tells (check against `.github/skills/debug/../chiron/references/ai-code-tells.md`)
