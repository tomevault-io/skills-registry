...
**Important:** Nano Banana Pro (Gemini API) is significantly slower than other image generation services:

| Resolution | Typical Generation Time |
|------------|------------------------|
| 1K | 2-3 minutes |
| 2K | 3-4 minutes |
| 4K | 4-6 minutes |
...
**Best Practices:**
- Set `timeout` to at least **120 seconds** when using `exec` tool
- The process may appear idle with no output for several minutes — this is normal
- Do NOT repeatedly poll or kill the process during generation
- Wait for the script to complete and output the file path

**Comparison with other services:**
- Seedream (Volcengine): ~30-60 seconds (faster, domestic servers)
- Nano Banana Pro (Gemini): ~3-4 minutes (slower, international API)
