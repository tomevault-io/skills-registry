---
name: auto-promote
description: 无人值守推广总入口。按 _promo/orchestrate.md 跨所有启用渠道（GitHub / Twitter / Reddit）跑一轮：发现→去重→起草→自我批判→护栏→按 posture 发布→记录→度量。供每日 cron（schedule / loop）调用，也可手动运行。传 "post-approved" 只执行 batch 待发队列。 Use when this capability is needed.
metadata:
  author: xiaotonng
---
