---
name: standard-team
description: 标准开发团队总指挥剧本（11 Phase 契约驱动 + 任务级 Dev-QA Loop）。当用户需要从需求到上线完整开发一个中型应用、说"用标准团队开发"、"完整开发新应用"、"按 11 Phase 跑"时激活。主会话作为调度者，按本剧本通过 Task 工具直接派遣 12 个成员 agent，不存在中间 orchestrator agent。 Use when this capability is needed.
metadata:
  author: xuanbingbingo
---

# 标准开发团队总指挥剧本

> **执行模型**：本 skill 是**主会话执行的剧本**，不是一个独立运行的 agent。
> 主会话读完本剧本后，用 `Task` 工具**直接**派遣 12 个成员 agent（按下方 Phase 0-11 顺序），可并行的并行。
>
...
- 不需要 Dev-QA Loop（纯结构性任务，无 UI 需要验证）
...
FOR 每个 project-tasks/frontend-tasklist.md 中的 [ ] 任务：
  ...
  STEP 2 - 调用 testing-evidence-collector 验证：
    验证内容（静态核查，不依赖浏览器/截图）：
      - UI 样式是否生效：CSS 静态核查（@media grep + viewport meta + 前端 typecheck）
      - API 调用字段名是否与契约一致
      - 表单提交和响应处理是否正确
      - 颜色/字体/间距是否使用了 CSS 变量（不得出现硬编码数值）
    产出：
      - PASS 或 FAIL + 可复核证据（typecheck/grep/单元测试输出，返回主会话）
      - **追加写入 docs/QA_REPORT.md**（与 PRD/SECURITY/REVIEW 同级，累计记录所有任务级 QA 结果）
...
---
> Source: [xuanbingbingo/claude-standard-dev-team](https://github.com/xuanbingbingo/claude-standard-dev-team) — distributed by [TomeVault](https://tomevault.io).
