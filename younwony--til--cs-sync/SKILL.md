...
CS-GUIDE.md는 계속 진화합니다. 새로운 필수 섹션(트러블슈팅, 키워드 등)이 추가되면 기존 문서와 형식이 불일치하게 됩니다.
수동으로 수십 개 문서를 하나씩 수정하면 누락이 발생하고 시간이 낭비됩니다.
이 스킬이 전체 문서를 자동 스캔하여 누락된 섹션을 찾고, 형식을 통일하고, 결과를 보고합니다.
...
4. **수동 검토**: 자동 동기화 후 사람이 직접 확인해야 오자, 문맥 오류, 의도하지 않은 변경을 잡을 수 있음
