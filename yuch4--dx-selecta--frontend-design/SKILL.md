...
description: 高品質でユニークなフロントエンドUIを作成する。「フロントエンドを作りたい」「UI作成」「Webページを作成」「コンポーネントを作って」「ランディングページ」などの依頼時に使用。AIっぽいジェネリックなデザインを避け、独自性のある実装コードを生成する。 Use when this capability is needed.
metadata:
  author: yuch4
---
...
創造的に解釈し、予想外の選択をする。
大胆なマキシマリズムも洗練されたミニマリズムも有効 - 重要なのは意図性。
