...
description: Generate a Hygon DCU HIP/C++ baseline kernel and correctness harness from a Torch, Triton, TileLang, Python, or CUDA/C++ reference plus shape JSON, including evidence-backed CUDA-to-HIP/DCU conversion, then hand the validated baseline to the Hygon HIP kernel optimizer. Use when the user has no initial HIP/C++ kernel, asks to start from a ref implementation, provides only ref.py and shape/dims, needs CUDA source ported to HIP/DCU, or wants automatic baseline generation before iterative DCU optimization. Use when this capability is needed.
metadata:
  author: yuguo-Jack
---
...
---
> Source: [yuguo-Jack/cuda-optimized-skill](https://github.com/yuguo-Jack/cuda-optimized-skill) — distributed by [TomeVault](https://tomevault.io).
