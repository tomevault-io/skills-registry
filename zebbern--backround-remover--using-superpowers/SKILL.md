...
**In VS Code / GitHub Copilot:** Read the skill file from `.superpowers/skills/[skill-name]/SKILL.md`. When you invoke a skill, its content is loaded and presented to you—follow it directly.

**Tool mappings:**
- `TodoWrite` → `manage_todo_list`
- `Task` with subagents → `runSubagent` tool
- `Skill` tool → Read skill file from `.superpowers/skills/`
...
Might any skill apply? (even 1% chance)
    ↓ yes                          ↓ definitely not
Read skill file                    Respond
    ↓
Announce: "Using [skill] to [purpose]"
    ↓
Has checklist?
    ↓ yes                          ↓ no
Create todo list per item          Follow skill exactly
    ↓
