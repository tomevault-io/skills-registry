...
description: 批量生成品牌Logo。支持两种入口：(1)用户描述品牌/用途后批量生成10个Logo变体 (2)用户上传参考图片后分析并生成Logo。提供7种主流风格选择。调用Gemini API生成，保存到Obsidian图片目录。触发词："生成Logo"、"设计Logo"、"做个Logo"、"批量Logo"、"帮我设计一个标志"。 Use when this capability is needed.
metadata:
  author: zephyrwang6
---
